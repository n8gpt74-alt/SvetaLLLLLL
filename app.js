...
(function () {
    ...
    const CATEGORY_ICONS = {
        ...
        health: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"></path></svg>',
        education: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"></path><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"></path></svg>',
        ...
        travel: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17.8 19.2 16 11l3.5-3.5C21 6 21.5 4 21 3c-1-.5-3 0-4.5 1.5L13 8 4.8 6.2c-.5-.1-.9.1-1.1.5l-.3.5c-.2.5-.1 1 .3 1.3L9 12l-2 3H4l-1 1 3 2 2 3 1-1v-3l3-2 3.5 5.3c.3.4.8.5 1.3.3l.5-.2c.4-.3.6-.7.5-1.2z"></path></svg>',
    ...
    function renderTags() {
        ...
        elements.tagsList.innerHTML = currentTags.map((tag, index) => `
            <div class="tag-badge">
                ${escapeHtml(tag)}
    ...
    function updateTodayNotes() {
        ...
        const todayItems = [
            ...state.items.notes.filter(n => n.date === todayStr).map(n => ({ ...n, type: 'notes' })),
            ...state.items.reminders.filter(r => r.date === todayStr).map(r => ({ ...r, type: 'reminders' }))
        ];

        if (todayItems.length === 0) {
            elements.todayNotesList.innerHTML = '<p class="no-notes">Нет заметок на сегодня</p>';
            return;
        }
        ...
        elements.todayNotesList.innerHTML = todayItems.map(item => {
            const category = state.categories.find(c => c.id === item.category);
            const color = category ? category.color : (item.type === 'notes' ? '#a855f7' : '#f97316');
            return `
    ...
    function renderCalendar() {
        ...
        for (let i = 0; i < totalCells; i++) {
            ...
            const isSelected = state.selectedDate && isSameDay(dateObj, state.selectedDate);
            ...
            let showIndicators = {
                notes: hasNotes && (state.activeCategory === 'all' || state.activeCategory === 'notes'),
                reminders: hasReminders && (state.activeCategory === 'all' || state.activeCategory === 'reminders')
            };

            let indicators = '';
            if (showIndicators.notes || showIndicators.reminders) {
                indicators = '<div class="day-indicators">';
                if (showIndicators.notes) indicators += '<span class="day-indicator notes"></span>';
                if (showIndicators.reminders) indicators += '<span class="day-indicator reminders"></span>';
                indicators += '</div>';
            }
    ...
    function renderPanelItems() {
        ...
        if (items.notes.length === 0) {
            elements.notesList.innerHTML = '<div class="empty-state">Нет заметок</div>';
        } else {
            elements.notesList.innerHTML = items.notes.map(note => renderItemCard(note, 'note')).join('');
        }

        if (items.reminders.length === 0) {
            elements.remindersList.innerHTML = '<div class="empty-state">Нет напоминаний</div>';
        } else {
            elements.remindersList.innerHTML = items.reminders.map(reminder => renderItemCard(reminder, 'reminder')).join('');
        }

        elements.notesList.querySelectorAll('.item-card').forEach(card => {
            addItemCardListeners(card, 'notes');
        });
        elements.remindersList.querySelectorAll('.item-card').forEach(card => {
            addItemCardListeners(card, 'reminders');
        });
    }
    ...
    function renderItemCard(item, type) {
        ...
        const category = state.categories.find(c => c.id === item.category);
        const categoryColor = category ? category.color : (type === 'note' ? '#a855f7' : '#f97316');
        ...
        return `
            <div class="item-card ${type} priority-${item.priority}" data-id="${item.id}" style="border-left-color: ${categoryColor}">
    ...
    function renderCategorySelect(selectedCategoryId = null) {
        elements.categorySelect.innerHTML = state.categories.map(cat => `
        ...
        elements.categorySelect.querySelectorAll('.category-select-item').forEach(item => {
            item.addEventListener('click', () => {
                elements.categorySelect.querySelectorAll('.category-select-item').forEach(i => i.classList.remove('selected'));
    ...
    function openModal(type, item = null) {
        ...
        elements.modalTitle.textContent = isEdit
            ? (isReminder ? 'Редактировать напоминание' : 'Редактировать заметку')
            : (isReminder ? 'Добавить напоминание' : 'Добавить заметку');
    ...
    function handleFormSubmit(e) {
        ...
        const selectedCategory = elements.categorySelect.querySelector('.category-select-item.selected');
        ...
        if (isEdit) {
            ...
            showToast(type === 'reminders' ? 'Напоминание обновлено' : 'Заметка обновлена', 'success');
            ...
            showToast(type === 'reminders' ? 'Напоминание добавлено' : 'Заметка добавлено', 'success');
        }
    ...
    function confirmDelete() {
        ...
        if (index !== -1) {
            ...
            renderCalendar();
            updateTodayNotes();
            showToast(type === 'reminders' ? 'Напоминание удалено' : 'Заметка удалена', 'success');
        }
    ...
    function renderCategoriesList() {
        ...
        elements.categoriesList.querySelectorAll('.category-item').forEach(item => {
            ...
            item.querySelector('.edit').addEventListener('click', () => openCategoryModal(catId));
            item.querySelector('.delete').addEventListener('click', () => deleteCategory(catId));
        });
    }
    ...
    function openCategoryModal(categoryId = null) {
        ...
        elements.categoryModalTitle.textContent = isEdit ? 'Редактировать категорию' : 'Новая категория';
        elements.categoryId.value = isEdit ? category.id : '';
        elements.categoryName.value = isEdit ? category.name : '';

        // Set color
        const colorRadio = document.querySelector(`input[name="categoryColor"][value="${isEdit ? category.color : '#ef4444'}"]`);
        if (colorRadio) colorRadio.checked = true;

        // Set icon
        const iconRadio = document.querySelector(`input[name="categoryIcon"][value="${isEdit ? category.icon : 'work'}"]`);
    ...
    function scheduleNotifications() {
        ...
        state.items.reminders.forEach(reminder => {
            if (!reminder.notify || !reminder.time || reminder.date !== todayStr) return;
            ...
            const notifyTime = new Date(reminderTime.getTime() - (reminder.notifyBefore || 0) * 60 * 1000);
    ...
    function setupEventListeners() {
        ...
        elements.addReminderBtn.addEventListener('click', () => openModal('reminders'));
        ...
        elements.datePanel.addEventListener('touchstart', (e) => {
            panelTouchStartY = e.changedTouches[0].screenY;
        }, { passive: true });

        elements.datePanel.addEventListener('touchend', (e) => {
